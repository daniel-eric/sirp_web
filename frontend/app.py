...
# Rota principal de comunicação do chat
@app.route('/api/chat', methods=['POST'])
def chat():
    dados = request.json or {}
    mensagem = dados.get('mensagem', '')
    midia_path = dados.get('midia_path', '')
    midia_tipo = dados.get('midia_tipo', '')
    
    # Acrescenta o metadado contextualizado da mídia caso ela exista para o histórico da IA
    if midia_path:
        mensagem_final = f"{mensagem} [Evidência anexada ({midia_tipo}): {midia_path}]"
    else:
        mensagem_final = mensagem

    chatbot.receber_mensagem(mensagem_final)
    
    return jsonify({
        "resposta_bot": chatbot.ultima_resposta_bot,
        "dossie_parcial": chatbot.detalhamento_problema,
        "finalizado": chatbot.finalizado
    })

# Rota para resetar a memória do bot quando a página recarregar
@app.route('/api/reset', methods=['POST'])
def reset_chat():
    global chatbot
    chatbot = ChatbotSIRP() # Recria a IA do zero, apagando o histórico antigo
    return jsonify({
        "status": "sucesso", 
        "mensagem_inicial": chatbot.ultima_resposta_bot
    })

# Lógica robusta de Upload e compressão em tempo real solicitado
    ...
    try:
        ...
        # 🎥 CONVERSÃO EXTREMA DE VÍDEOS VIA FFMPEG (VP9/WebM)
            ...
            try:
                ...
                # Se o FFmpeg do sistema falhar ou não estiver configurado, mantém o original para segurança do fluxo
